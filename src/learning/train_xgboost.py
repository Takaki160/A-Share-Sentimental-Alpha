...
CONFIG = {
    ...
    "XGB_PARAMS": {
        ...
        'booster': 'gbtree',
        'n_estimators': 300,
        'learning_rate': 0.02,        # 降低学习率，更稳健
        'max_depth': 3,               # 降低深度 (原来是4)，防止过拟合
        'min_child_weight': 5,
        'subsample': 0.8,
        'colsample_bytree': 0.8,
        'reg_alpha': 1.0,             # 增加正则化 (原来是0.5)
...
def get_top_features():
    ...
    logging.info(f"💎 Selected Top {CONFIG['TOP_N_FEATURES']} Features for XGBoost:")
...
def run_backtest(df, features):
    ...
    logging.info(f"Starting Optimized XGBoost Rolling ({current_date.date()} -> {end_date.date()})")
...
def main():
    print(f"🚀 Starting XGBoost (Top {CONFIG['TOP_N_FEATURES']} Factors)")
    ...
    plt.title(f'XGBoost: Top {CONFIG["TOP_N_FEATURES"]} Features')
    fig.tight_layout()
    plt.savefig(REPORTS_DIR / "backtest_xgboost.png", dpi=300)
